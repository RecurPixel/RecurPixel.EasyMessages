...
public static class MessageRegistry
{
    ...
    public static void LoadCustomMessages(string jsonPath)
    {
        if (!File.Exists(jsonPath))
        {
            throw new FileNotFoundException(
                $"Custom message file not found at path: {jsonPath}");
        }
        ...
        lock (_lock)
        {
            try
            {
                ...
                Console.WriteLine($"Successfully loaded {_custom.Count} custom message(s) from '{jsonPath}'");
            }
    ...
    internal static void LoadCustomMessages(Dictionary<string, MessageTemplate> messages)
    {
        ...
        lock (_lock)
        {
            ...
            Console.WriteLine($"Successfully loaded {_custom.Count} custom message(s) from dictionary");
        }
    }
    ...
    private static void LoadCustomMessagesInternal(Dictionary<string, MessageTemplate> messages)
    {
        ...
        if (warnings.Any())
        {
            Console.WriteLine($"Note: {warnings.Count} custom message code(s) not found in defaults:\n" +
                            string.Join("\n", warnings.Select(w => $"  - {w}")));
        }
    }

    private static MessageTemplate MergeTemplates(MessageTemplate defaultTemplate, MessageTemplate customTemplate)
    {
