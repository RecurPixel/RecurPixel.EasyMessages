...
| **Console Output** | ✅ `.ToConsole()` | ⚠️ Available but not typical |
| **JSON/XML Output** | ✅ `.ToJson()`, `.ToXml()` | ✅ `.ToJson()`, `.ToXml()` |
| **API Response** | ❌ Not available | ✅ `.ToApiResponse()` |
| **Logging Integration** | ⚠️ Manual only | ✅ `.Log(ILogger)` |
...
| **Correlation IDs** | ⚠️ Manual only | ✅ Automatic |
...
💡 **Note:** This automatically includes the Core package.
...
⚠️ **Console output** - Available but not typical for APIs
⚠️ **Manual file saving** - APIs return responses, not save files
